...
export const GOOGLE_ROLE_BLOCKED_MESSAGE =
  "Login Google tidak diizinkan untuk peran akun Anda.";
...
export function inferGooglePortal(callbackUrl: string | null | undefined): "student" | "staff" {
  ...
  try {
    const path = callbackUrl.startsWith("http") ? new URL(callbackUrl).pathname + new URL(callbackUrl).search : callbackUrl;
...
export function googleAllowedEmailDomains(): Set<string> {
  ...
  const list = raw
    ? raw.split(",")
    : [process.env.NEXT_PUBLIC_STAFF_DOMAIN, process.env.NEXT_PUBLIC_STUDENT_DOMAIN];
  return new Set(
    list
